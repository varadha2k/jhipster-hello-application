...
import com.varadha2k.hello.config.KafkaProperties;
...
class JhipsterhelloKafkaResourceIT {
    ...
    void setup() {
        ...
        JhipsterhelloKafkaResource kafkaResource = new JhipsterhelloKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jhipsterhello-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipsterhello-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
